...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, enc_output: torch.Tensor, tgt_mask: torch.Tensor) -> torch.Tensor:
        ...
        attn_out = self.self_attention(hidden_state, hidden_state, hidden_state, tgt_mask)  # <--- sin keyword
        x = x + attn_out

        # Apply layer normalization and cross-attention
        hidden_state = self.layer_norm_2(x)
        cross_out = self.cross_attention(hidden_state, enc_output, enc_output, mask=None)
        x = x + cross_out
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, enc_output: torch.Tensor) -> torch.Tensor:
        ...
        tgt_mask = torch.tril(torch.ones((seq_len, seq_len), device=x.device, dtype=torch.bool)).unsqueeze(0)
